...
export const getDashboardStats = async (req, res) => {
    try {
        const userId = req.body.user.userId; // Assuming auth middleware adds user ID to req.user
        const currentYear = new Date().getFullYear();
        const startOfYear = new Date(currentYear, 0, 1);
        const endOfYear = new Date(currentYear + 1, 0, 1);

        // Total Users Count for current year
        const totalUsers = await Users.countDocuments({
            createdAt: { $gte: startOfYear, $lt: endOfYear }
        });

        // Active Jobs Count
        const activeJobs = await Jobs.countDocuments({ company: userId });

        // Applicants Count for jobs posted by current user's company in the current year
        const applicants = await JobApplications.countDocuments({
            jobId: { $in: (await Jobs.find({ company: userId })).map(job => job._id) },
            createdAt: { $gte: startOfYear, $lt: endOfYear }
        });

        // Interviews Count for applicants with 'interviewScheduled' status
        const interviews = await Interviews.countDocuments({
            applicationId: {
                $in: (await JobApplications.find({
                    jobId: { $in: (await Jobs.find({ company: userId })).map(job => job._id) },
        ...
        // Graph data: Monthly breakdown of Users and Applicants
        const graphData = {
            labels: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'],
            users: await getMonthlyData(Users, startOfYear, endOfYear),
            applicants: await getMonthlyData(JobApplications, startOfYear, endOfYear, { jobId: { $in: (await Jobs.find({ company: userId })).map(job => job._id) } })
        };
...
// Helper function to get monthly counts for a model
const getMonthlyData = async (Model, startOfYear, endOfYear, query = {}) => {
    const pipeline = [
        { $match: { ...query, createdAt: { $gte: startOfYear, $lt: endOfYear } } },
        {
            $group: {
                _id: { $month: '$createdAt' },
                count: { $sum: 1 }
